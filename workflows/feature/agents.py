"""The five seats of `feature`: Claude Code, in the dev container.

Each seat is :class:`~workflows.shared.agents.SandboxAgent` — how the
agent is reached, what it runs as and how long it may take are the
sandbox's, set once there — plus only its prompt and, where the node
routes on a verdict, the model that verdict must fit.
"""
...
from workflows.shared.sandbox import AGENT_SH, GATE_COMMAND
