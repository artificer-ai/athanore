...
| 23 | [Session continuity](23-session-continuity.md) | The third post-1.0 phase: one ACP session continued across several agent runs — `session_id=` on the façade, `session/resume` over `session/load`, the replay kept out of the transcript, what the stats entry may and may not claim, and the fake persisting sessions |
