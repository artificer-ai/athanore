...
athanore/                      Python package (distribution "athanore")
  ...
  api/                         (08)
    app.py                     create_app(settings, engine, store, plugins, registrar)
    registrar.py               WorkflowRegistrar: the port the live-registration routes call the host through (22 §Wire)
...
| `plugin_cdns` | jsdelivr, unpkg, esm.sh | Origins a plugin pane may fetch scripts, styles and fonts from, on top of `'self'`. `[]` is the airtight policy; `["https:"]` opens it to any origin (D211) |
