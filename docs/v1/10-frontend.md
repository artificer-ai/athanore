...
| Client state | zustand (selection, pane index, list width/collapsed, preferences) | Persisted to `localStorage` |
...
| Icons | Phosphor (`@phosphor-icons/react`) | Nocturne's icon set; the mock mostly uses glyphs, which are kept |
...
3. **agent** — `AGENT STREAM · task N · node · streaming/finished ·
   node → adapter`. Blocks by kind: `system` (neutral-600 label),
...
- **Task drawer**: click a NODES row (overview) or a graph row to open
  the attempt's detail (payload, result, error, submissions, stats,
  lineage) with retry / move / set-status actions.
...
mock's primitives (metric tiles, zebra tables, kicker labels).
...
- A new request on the selected run switches the pane to **agent** once
  (opt-out in settings). The browser tab title prefixes the open-request
  count; desktop notifications are opt-in.
...
| Status pill | `Badge variant="outline"` with the status colour |
...
| Inputs | `Input` / `Textarea` on `--color-bg` with neutral-800 borders |
