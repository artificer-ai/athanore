...
| ☑ | `test_permissions.py` | `tests/agents/test_policies.py` (the policy assertions: reject-first ordering under `auto_allow`, `auto_deny`, a policy that cannot be honoured, the `ask` request and its options, the engine-authored timeout answer, the bounded tool-call summary, done); `tests/agents/test_acp_outcomes.py` still carries the round trip over the ACP wire | T038, T039a |
| ☑ | `test_elicitation.py` | `tests/agents/test_policies.py` (the form request and its registered validator, the accept, and the declines — policy, URL mode, no schema, no context, timeout — done; the MVP's `test_schema_validator` is `tests/requests/test_validators.py`'s from T030); `tests/agents/test_acp_outcomes.py` still carries the end-to-end elicitation over ACP | T038, T039a |
