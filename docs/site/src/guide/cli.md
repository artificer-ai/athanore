...
that are thin wrappers over the HTTP API. The browser interface is the
full operator interface and the command line is deliberately not, but it
is enough to start a server, submit work, look at a run and answer a
question from a shell or a script.
