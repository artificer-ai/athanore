...
[published with this documentation](../openapi.json).
...
is a 403. An unknown token, another task's token, and an attempt that has
ended share one message, because the server does not tell a caller which
of the three it was.
...
Three things to know before you write a consumer.
...
## Body size limits and pagination

JSON bodies are capped, by default at one mebibyte, and a body over the
cap is refused with `payload_too_large`, whether or not it declared its
length. Lists that can grow take `limit` and `cursor`; run lists are
small and come back whole. Timestamps are ISO-8601 in UTC.
