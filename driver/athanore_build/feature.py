...
class ImplementerAgent(SandboxAgent):
    ...
    system_prompt = (
        ...
        "whether your own work passed. Run it in the foreground and wait "
        "for it — it takes roughly 25 minutes, and that is expected. Do "
        "not background it and poll for completion: a `pgrep -f` whose "
...
class QAAgent(SandboxAgent):
    ...
    system_prompt = (
        ...
        "- drive the SPA with chromium directly when there is a page to "
        "open. Playwright's npm package is not installed until T068a, but "
        "the browser is, at "
        "`$PLAYWRIGHT_BROWSERS_PATH/chromium-*/chrome-linux64/chrome`: "
        "`--headless --no-sandbox --dump-dom <url>` gives the post-JS "
        "DOM, `--screenshot=<path>` captures a frame, and "
        "`--remote-debugging-port=9222` exposes CDP for computed styles "
        "— the check that catches invisible text, wrong colours and "
        "collapsed panes, none of which jsdom can see;\n"
