...
export function RunFilters({ workflows }: { workflows: readonly string[] }) {
  ...
  return (
    <>
      ...
      >
        {[ALL_WORKFLOWS, ...workflows].map((workflow) => (
          <ToggleGroup.Item
            key={workflow}
            value={workflow}
            className="rounded-lg border border-border px-[8px] py-[3px] text-[calc(10.5rem/12)] text-muted-foreground hover:border-[var(--color-accent-600)] hover:text-[var(--color-accent-200)] data-[state=on]:border-[var(--color-accent-600)] data-[state=on]:bg-accent data-[state=on]:text-[var(--color-accent-200)]"
          >
            {workflow}
          </ToggleGroup.Item>
        ))}
      ...
      <div className="flex min-w-[190px] items-center gap-[8px] rounded-lg border border-border bg-card px-[8px] py-[4px]">
        ...
        <input
          ...
          className="text-meta w-full bg-transparent text-foreground outline-none placeholder:text-[var(--color-neutral-600)]"
        />
      </div>
    </>
  )
