...
every schema are in [HTTP API](http-api.md), generated from
...
The vocabulary is in [Error codes](errors.md). Status codes
...
- Deployment (see the athanore-cli skill): putting this behind a proxy, with a
  token.
- [HTTP API](http-api.md): every operation and schema.
